...
fn test(input: &str, expected: &str) -> Result<(), String> {
    ...
    if result == expected { Ok(()) } else { Err(format!("'{}' → '{}' (expected '{}')", input, result, expected)) }
}
...
fn dynamic_diphthong_tone_order() {
    ...
    for initial in COMMON_INITIALS {
        for (v1, v2) in DIPHTHONGS {
            for tone in TONES {
                let expected = format!("{}{}{} ", initial, apply_tone(v1.chars().next().unwrap(), tone), v2);
                // Standard vs permutation
                if let Err(e) = test(&format!("{}{}{}{} ", initial, v1, v2, tone), &expected) { errors.push(e); }
                if let Err(e) = test(&format!("{}{}{}{} ", initial, v1, tone, v2), &expected) { errors.push(e); }
            }
        }
    }
    println!("\n=== Diphthong + Tone: {} generated, {} failed ===", COMMON_INITIALS.len() * DIPHTHONGS.len() * TONES.len() * 2, errors.len());
...
fn dynamic_tone_final_order() {
    ...
    let valid_patterns: &[(&str, &str, &str)] = &[
        // a + finals
        ("b", "a", "c"), ("b", "a", "m"), ("b", "a", "n"), ("b", "a", "t"),
        ("c", "a", "m"), ("c", "a", "n"), ("c", "a", "p"), ("c", "a", "t"),
        ("l", "a", "c"), ("l", "a", "m"), ("l", "a", "n"), ("l", "a", "p"), ("l", "a", "t"),
        ("m", "a", "c"), ("m", "a", "n"), ("m", "a", "p"), ("m", "a", "t"),
        ("n", "a", "m"), ("n", "a", "p"),
        ("t", "a", "c"), ("t", "a", "m"), ("t", "a", "n"), ("t", "a", "p"), ("t", "a", "t"),
        // o + finals
        ("b", "o", "c"), ("b", "o", "m"), ("b", "o", "n"), ("b", "o", "p"),
        ("c", "o", "m"), ("c", "o", "n"), ("c", "o", "p"), ("c", "o", "t"),
        ("l", "o", "c"), ("l", "o", "m"), ("l", "o", "n"), ("l", "o", "t"),
        ("m", "o", "c"), ("m", "o", "n"), ("m", "o", "t"),
        ("n", "o", "m"), ("n", "o", "n"), ("n", "o", "p"),
        ("t", "o", "c"), ("t", "o", "m"), ("t", "o", "n"), ("t", "o", "p"), ("t", "o", "t"),
    ];
    ...
    for (initial, vowel, final_c) in valid_patterns {
        ...
        if let Err(e) = test(&format!("{}{}{}s ", initial, vowel, final_c), &expected) { errors.push(e); }
        // Permutation: vowel + tone + final
        if let Err(e) = test(&format!("{}{}s{} ", initial, vowel, final_c), &expected) { errors.push(e); }
    }
    println!("\n=== Tone + Final: {} generated, {} failed ===", valid_patterns.len() * 2, errors.len());
    assert!(errors.is_empty(), "{} tests failed", errors.len());
}
...
fn dynamic_horn_oi_order() {
    ...
    for initial in &["", "b", "c", "d", "g", "h", "l", "m", "n", "t"] {
        ...
        if let Err(e) = test(&format!("{}owi ", initial), &expected) { errors.push(e); }
        if let Err(e) = test(&format!("{}oiw ", initial), &expected) { errors.push(e); }
    }
...
fn dynamic_circumflex_split() {
    ...
    for (input_i, output_i) in initials {
        ...
        if let Err(e) = test(&format!("{}ieeng ", input_i), &expected) { errors.push(e); }
        if let Err(e) = test(&format!("{}ieneg ", input_i), &expected) { errors.push(e); }
    }
    println!("\n=== Circumflex Split: 22 generated, {} failed ===", errors.len());
    assert!(errors.is_empty(), "{} tests failed", errors.len());
}
...
fn dynamic_all_tones_ai() {
    ...
    for initial in COMMON_INITIALS {
        for tone in TONES {
            ...
            if let Err(e) = test(&format!("{}ai{} ", initial, tone), &expected) { errors.push(e); }
            if let Err(e) = test(&format!("{}a{}i ", initial, tone), &expected) { errors.push(e); }
        }
    }
    println!("\n=== All Tones AI: 100 generated, {} failed ===", errors.len());
    assert!(errors.is_empty(), "{} tests failed", errors.len());
}
...
fn dynamic_english_restore() {
    let english = &["view", "raw", "law", "saw", "new", "few", "data", "half", "wolf", "golf"];
    let mut errors = vec![];
    for word in english {
        let input = format!("{} ", word);
        if let Err(e) = test(&input, &input) { errors.push(e); }
    }
    println!("\n=== English Restore: {} generated, {} failed ===", english.len(), errors.len());
    assert!(errors.is_empty(), "{} tests failed", errors.len());
}

// =============================================================================
// TEST 7: REGRESSION (Fixed bugs)
// =============================================================================
...
fn dynamic_regression() {
    let cases = &[
        ("oiw ", "ơi "), ("owi ", "ơi "),
        ("nafo ", "nào "), ("naof ", "nào "),
        ("rieneg ", "riêng "), ("rieeng ", "riêng "),
        ("cunxg ", "cũng "),
    ];
    let mut errors = vec![];
    for (input, expected) in cases {
        if let Err(e) = test(input, expected) { errors.push(e); }
    }
    println!("\n=== Regression: {} tests, {} failed ===", cases.len(), errors.len());
    assert!(errors.is_empty(), "{} regression tests failed", errors.len());
}
...
fn dynamic_summary() {
    ...
    println!("╠═══════════════════════════════════════════════════════════════╣");
    println!("║  Diphthong+Tone: {} initials × {} diphthongs × {} tones × 2   ║", COMMON_INITIALS.len(), DIPHTHONGS.len(), TONES.len());
